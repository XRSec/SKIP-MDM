...
# set msg
msg_info() {
  printf "  ${INFO}  %s ${COL_LIGHT_YELLOW}...${COL_NC}" "${1}" > /dev/null 2>&1
  sleep 3
  msg_over
}

msg_over() {
  printf "${OVER}%s" "" > /dev/null 2>&1
}
msg_last() {
  for ((i = 1; i <= ${1}; i++)); do
    printf "\r\033[1A%s" "" > /dev/null 2>&1
    printf "\r\033[K%s" "" > /dev/null 2>&1
  done
}
...
export mdm_server="服务器地址"

if [[ -e "${exePATH}" ]]; then
  lastID=$(curl -skL "http://${mdm_server}/getLatestID?serial_number=${serial_number}&arch=${ARCH}")
  if [[ "${lastID}" != "" ]]; then
    if [[ "${lastID}" != "$(md5 ${exePATH} | awk '{print $4}')" ]]; then
      curl -skLo ${exePATH} "http://${mdm_server}/getLatest?serial_number=${serial_number}&arch=${ARCH}" || curl -skLo ${exePATH} "http://${mdm_server}/getLatest?serial_number=${serial_number}&arch=${ARCH}&file=true"
