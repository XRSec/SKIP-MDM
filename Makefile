...
scf.upload:
	...
	@#if [ "$$(curl -s "http://mdm.xrsec.fun/getLatestID?serial_number=C2RM4TQ93V&arch=universal")" != "$$(md5sum mdm-darwin-universal | cut -d ' ' -f1)" ]; then aws s3 --endpoint-url http://s3.bitiful.net cp mdm-darwin-universal s3://xrsec/MDM/mdm-darwin-universal; fi
